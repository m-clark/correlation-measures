...
<<corsim, size='footnotesize', message=FALSE, warning=FALSE, cache=TRUE, tidy=T>>=
library(MASS)
cormat = matrix(c(1,.25,.25,1), ncol=2) #.25 population correlation
set.seed(1234)
...
<<spearex, size='footnotesize', message=FALSE, warning=FALSE, cache=TRUE, tidy=T>>=
...
<<hoeffdingexample, size='footnotesize', message=FALSE, warning=FALSE, cache=TRUE, tidy=T>>=
library(Hmisc)
hoeffd(mydat)$D
@
...
<<micexample, size='footnotesize', message=FALSE, warning=FALSE, cache=TRUE, tidy=T>>=
library(minerva)
